...
add_action('init', function () {
    if (function_exists('pll_register_post_type')) {
        pll_register_post_type('location');
    }
    if (function_exists('pll_register_taxonomy')) {
        pll_register_taxonomy('location_category');
    }
});
...
function render_location_cards_box($post)
{
    wp_nonce_field('save_location_cards', 'location_cards_nonce');

    $cards = get_post_meta($post->ID, 'location_cards', true);
    ...
    <div id="location-cards-wrapper">
        <?php foreach ($cards as $card_index => $card): ?>
            <div class="location-card">
                <!-- Card Title -->
                <input class="widefat" type="text" name="location_cards[<?php echo $card_index; ?>][title]"
                    value="<?php echo esc_attr($card['title'] ?? ''); ?>" placeholder="Card Title" />

                <!-- Short Content (outer card) -->
                <textarea class="widefat" rows="3" name="location_cards[<?php echo $card_index; ?>][short_content]" placeholder="Short Content"><?php echo esc_textarea($card['short_content'] ?? ''); ?></textarea>

                <div class="location-tabs">
                    <?php if (!empty($card['tabs']) && is_array($card['tabs'])):
                        foreach ($card['tabs'] as $tab_index => $tab): ?>
                            <div class="location-tab">
                                <!-- Tab Title -->
                                <input class="widefat" type="text"
                                    name="location_cards[<?php echo $card_index; ?>][tabs][<?php echo $tab_index; ?>][title]"
                                    value="<?php echo esc_attr($tab['title'] ?? ''); ?>" placeholder="Tab Title" />

                                <!-- Details Page Link -->
                                <input class="widefat" type="text"
                                    name="location_cards[<?php echo $card_index; ?>][tabs][<?php echo $tab_index; ?>][details_link]"
                                    value="<?php echo esc_attr($tab['details_link'] ?? ''); ?>" placeholder="Details Page Link (URL)" />

                                <!-- Image Upload -->
                                <div class="image-field">
                                    <input type="hidden"
                                        name="location_cards[<?php echo $card_index; ?>][tabs][<?php echo $tab_index; ?>][image]"
                                        value="<?php echo esc_attr($tab['image'] ?? ''); ?>" class="tab-image-url" />
                                    <div class="image-actions">
                                        <button type="button" class="button select-image">Select Image</button>
                                        <button type="button" class="button remove-image" <?php echo empty($tab['image']) ? 'style="display:none;"' : ''; ?>>Remove Image</button>
                                    </div>
                                ...
                                <!-- WYSIWYG Editor -->
                                <?php
                                $editor_id = "editor_" . $card_index . "_" . $tab_index;
                                wp_editor($tab['content'] ?? '', $editor_id, [
                                    'textarea_name' => "location_cards[{$card_index}][tabs][{$tab_index}][content]",
                                    'textarea_rows' => 4,
                                    'teeny' => true,
                                    'media_buttons' => false
                                ]);
                                ...
                                <button type="button" class="button remove-tab">Remove Tab</button>
                            </div>
                        <?php endforeach;
                    endif; ?>
                </div>

                <button type="button" class="button add-tab">+ Add Tab</button>
                <button type="button" class="button remove-card">Remove Card</button>
            </div>
        <?php endforeach; ?>
    </div>
    <button type="button" id="add-card" class="button button-primary">+ Add Card</button>
    ...
    <style>
        ...
        .image-actions { display: flex; gap: 6px; margin-bottom: 8px; }
        .image-actions .button { flex-shrink: 0; }
        .image-preview img {
            max-width: 150px;
            border: 1px solid #ccc;
            padding: 3px;
    ...
    <script>
        jQuery(document).ready(function ($) {
            let cardWrapper = $('#location-cards-wrapper');

            $('#add-card').on('click', function () {
                let cardCount = cardWrapper.find('.location-card').length;
                let cardHTML = `
                    <div class="location-card">
                        <input class="widefat" type="text" name="location_cards[${cardCount}][title]" placeholder="Card Title" />
                        <textarea class="widefat" rows="3" name="location_cards[${cardCount}][short_content]" placeholder="Short Content"></textarea>
                        <div class="location-tabs"></div>
                        <button type="button" class="button add-tab">+ Add Tab</button>
                        <button type="button" class="button remove-card">Remove Card</button>
                    </div>
                `;
            ...
            cardWrapper.on('click', '.remove-card', function () {
                $(this).closest('.location-card').remove();
            });

            cardWrapper.on('click', '.add-tab', function () {
                let card = $(this).closest('.location-card');
                let cardIndex = card.index();
                let tabCount = card.find('.location-tab').length;
                let tabHTML = `
                    <div class="location-tab">
                        <input class="widefat" type="text" name="location_cards[${cardIndex}][tabs][${tabCount}][title]" placeholder="Tab Title" />
                        <input class="widefat" type="text" name="location_cards[${cardIndex}][tabs][${tabCount}][details_link]" placeholder="Details Page Link (URL)" />
                        <div class="image-field">
                            <input type="hidden" name="location_cards[${cardIndex}][tabs][${tabCount}][image]" class="tab-image-url" />
                            <div class="image-actions">
                                <button type="button" class="button select-image">Select Image</button>
                        ...
                        <textarea name="location_cards[${cardIndex}][tabs][${tabCount}][content]" class="widefat" rows="4" placeholder="Tab Content"></textarea>
                        <button type="button" class="button remove-tab">Remove Tab</button>
                    </div>
                `;
                card.find('.location-tabs').append(tabHTML);
            });

            cardWrapper.on('click', '.remove-tab', function () {
                $(this).closest('.location-tab').remove();
            });

            cardWrapper.on('click', '.select-image', function (e) {
                e.preventDefault();
                let button = $(this);
                let customUploader = wp.media({
                    ...
                    let container = button.closest('.image-field');
                    container.find('.tab-image-url').val(attachment.url);
                    container.find('.image-preview').html('<img src="' + attachment.url + '">');
                    button.siblings('.remove-image').show();
                }).open();
            });

            cardWrapper.on('click', '.remove-image', function () {
                let container = $(this).closest('.image-field');
                container.find('.tab-image-url').val('');
...
add_action('save_post', function ($post_id) {
    if (!isset($_POST['location_cards_nonce']) || !wp_verify_nonce($_POST['location_cards_nonce'], 'save_location_cards'))
        return;
    ...
    if (isset($_POST['location_cards']) && is_array($_POST['location_cards'])) {
        update_post_meta($post_id, 'location_cards', $_POST['location_cards']);
    } else {
        delete_post_meta($post_id, 'location_cards');
    }
});
